...
void dxdt(double *du, double *dv, const double *u, const double *v) {
  ...
  #pragma omp parallel for default(none) shared(u,v,du,dv,M,N,DD,R,d) private(idx,i,j,up,down,left,right,lapu,lapv,_u,_v)
  for (i = 1; i < M-1; i++) {
    for (j = 1; j < N-1; j++) {
      idx = i * N + j;
      down = (i - 1) * N + j;
...
int main(int argc, char **argv) {
  ...
  // write norms output
  char filename[50];
  sprintf(filename, "%d_cores_part1_dxdt_default.dat", omp_get_max_threads());
