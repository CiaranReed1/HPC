...
void dxdt(double *du, double *dv, const double *u, const double *v) {
  ...
  #pragma omp for nowait private(i,j,up,down,left,right,idx,_u,_v,lapu,lapv)//left and right edges (excluding corners)
  for (i = 1; i < M-1; i++) { 
  ...
  #pragma omp for nowait private(i,j,up,down,left,right,idx,_u,_v,lapu,lapv) //bottom and top edges
  for (j = 0; j < N; j++) { 
  ...
  #pragma omp for private(i,j,up,down,left,right,idx,_u,_v,lapu,lapv) //interior points
  for (i = 1; i < M-1; i++) {
    for (j = 1; j < N-1; j++) {
      idx = i * N + j;
      down = (i - 1) * N + j;
  ...
  #pragma omp parallel default(none) shared(u,v,du,dv,stats,k,t,nrmu,nrmv,writeInd,M,N,T,m,dt,R,d,DD)
  {
    for (k = 0; k < T; k++) {
      ...
      if (k % m == 0) { // every m time steps, store norms
        ...
        } //ensures all threads have finished updating u and v before calculating norms
        stats[writeInd][0] = t;
