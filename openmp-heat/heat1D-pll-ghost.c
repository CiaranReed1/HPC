...
void step(double *u, int N){

	u[0] = (u[0]+u[1])/2.0;		//unew[0] 	= (u[0] + u[1])/2.0;			// alternative boundary conditions
	#pragma omp parallel 
	{
		#pragma omp for 
	...
	u[N-1] = (u[N-1] + u[N-2])/2.0;	//unew[N-1]	= (u[N-1] + u[N-2])/2.0;	// alternative boundary conditions
}
